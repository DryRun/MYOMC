...
if [ -r CMSSW_10_6_4_wmLHEGEN/src ] ; then 
    echo release CMSSW_10_6_4_wmLHEGEN already exists
	cd CMSSW_10_6_4_wmLHEGEN/src
	eval `scram runtime -sh`
else
    scram project -n "CMSSW_10_6_4_wmLHEGEN" CMSSW_10_6_4
	cd CMSSW_10_6_4_wmLHEGEN/src
	eval `scram runtime -sh`
fi
...
cmsDriver.py Configuration/GenProduction/python/HIG-RunIISummer19UL17wmLHEGEN-00007-fragment.py \
	--fileout "file:RunIISummer19UL17wmLHEGEN_$NAME_$JOBINDEX.root" \
	--mc \
	...
	--era Run2_2017 \
	--python_filename "RunIISummer19UL17wmLHEGEN_${NAME}_cfg.py" \
	--no_exec \
	--customise Configuration/DataProcessing/Utils.addMonitoring \
	--customise_commands process.source.numberEventsInLuminosityBlock="cms.untracked.uint32(5000)" \
	-n $NEVENTS
cmsRun "RunIISummer19UL17wmLHEGEN_${NAME}_cfg.py"
if [ ! -f "RunIISummer19UL17wmLHEGEN_$NAME_$JOBINDEX.root" ]; then
	echo "RunIISummer19UL17wmLHEGEN_$NAME_$JOBINDEX.root not found. Exiting."
	return 1
fi

# SIM
export SCRAM_ARCH=slc7_amd64_gcc700
if [ -r CMSSW_10_6_2_SIM/src ] ; then 
    echo release CMSSW_10_6_2_SIM already exists
	cd CMSSW_10_6_2_SIM/src
	eval `scram runtime -sh`
else
    scram project -n "CMSSW_10_6_2_SIM" CMSSW_10_6_2
	cd CMSSW_10_6_2_SIM/src
	eval `scram runtime -sh`
fi
cd $CMSSW_BASE/src
...
cd $TOPDIR
cmsDriver.py step1 \
	--filein "file:RunIISummer19UL17wmLHEGEN_${NAME}_cfg.py" \
	--fileout "RunIISummer19UL17SIM_${NAME}_cfg.py" \
	--mc \
	--eventcontent RAWSIM \
	--runUnscheduled \
	--datatier GEN-SIM \
	...
	--geometry DB:Extended \
	--era Run2_2017 \
	--python_filename "RunIISummer19UL17SIM_${NAME}_cfg.py" \
	--no_exec \
	--customise Configuration/DataProcessing/Utils.addMonitoring \
	-n $NEVENTS
cmsRun "RunIISummer19UL17SIM_${NAME}_cfg.py"
if [ ! -f "RunIISummer19UL17SIM_$NAME_$JOBINDEX.root" ]; then
	echo "RunIISummer19UL17SIM_$NAME_$JOBINDEX.root not found. Exiting."
	return 1
fi
